        ...
        for b in batches:
            ...
            if generator is None:
                ...
                with torch.no_grad(), gen_measures:
                    gen_out = generator(b['text'], **gen_kw)
                    mel, mel_lens = gen_out[0], gen_out[1]
                    pitch = gen_out[3] if len(gen_out) > 3 else None
                ...
                if args.save_mels:
                    for i, mel_ in enumerate(mel):
                        ...
                        # Guardar pitch predicho
                        if pitch is not None:
                            p = pitch[i, :, :mel_lens[i].item()].squeeze(0)
                            pitch_path = Path(args.output, Path(fname).stem + '_pitch.npy')
                            np.save(pitch_path, p.cpu().numpy())
            ...
            if vocoder is not None:
                ...
                if args.output is not None and reps == 1:
                    for i, audio in enumerate(audios):
                        ...
                        if audio.numel() > 0 and torch.max(torch.abs(audio)) > 0:
