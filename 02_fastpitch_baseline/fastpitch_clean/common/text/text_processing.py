...
def get_text_processing(symbol_set, text_cleaners, p_arpabet):
    if symbol_set in ['english_basic', 'english_basic_lowercase', 'english_expanded']:
        ...
        return IPATextProcessing(symbol_set)
    else:
        raise ValueError(f"No TextProcessing for symbol set {symbol_set} unknown.")


class IPATextProcessing:
    """Procesador simple para IPA pre-tokenizado (espacio separado)."""
    ...
    def encode_text(self, text):
        ...
        ids = []
        for t in tokens:
            if t in self.symbol_to_id:
                ids.append(self.symbol_to_id[t])
            else:
                print(f"WARNING: token IPA desconocido: {t!r}")
        return ids

    def sequence_to_text(self, sequence):
        return " ".join([self.symbols[i] for i in sequence if i < len(self.symbols)])
